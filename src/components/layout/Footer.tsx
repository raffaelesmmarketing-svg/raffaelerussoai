...
const socials = [
  {
    label: 'Instagram',
    href: 'https://instagram.com/raffaelerussoai',
    ...
    <footer className="border-t border-white/[0.08] pt-20 pb-8 bg-navy-900">
      <div className="max-w-[1200px] mx-auto px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-[2fr_1fr_1fr_1fr] gap-12 mb-16">
            ...
            <div className="flex gap-2">
              {socials.map((s) => (
                <a
                  ...
                  aria-label={s.label}
                  className="w-9 h-9 rounded-full inline-flex items-center justify-center bg-white/[0.04] border border-white/[0.08] text-fog-300 hover:text-lime-500 hover:border-lime-500 hover:bg-lime-500/10 transition-colors duration-200"
                >
          ...
          {cols.map((c) => (
            <div key={c.title}>
              <div className="font-mono text-[11px] tracking-[0.16em] uppercase text-fog-500 font-bold mb-4">
                {c.title}
              </div>
              <ul className="flex flex-col gap-2.5">
                {c.links.map((item) => (
                  <li key={item.l}>
                    <Link
                      href={item.href}
                      className="font-body text-sm text-fog-300 no-underline hover:text-lime-500 transition-colors"
                    >
                      {item.l}
                    </Link>
                  </li>
        ...
        <div className="pt-6 border-t border-white/[0.08] flex justify-between items-center flex-wrap gap-4">
          <div className="font-mono text-xs text-fog-500">© 2026 Raffaele Russo · Lisbona</div>
          <div className="font-mono text-xs text-fog-500 italic">
            &quot;<em className="text-lime-500">Spingere</em>.&quot;
          </div>
        </div>
      </div>
    </footer>
